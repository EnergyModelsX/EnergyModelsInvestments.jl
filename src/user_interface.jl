...
function run_model(fn, model, optimizer=nothing)
 ...
 function read_data(fn)
    ...
    nodes = [
            ...
            EMB.RefSource(3, FixedProfile(0), FixedProfile(30), FixedProfile(100), Dict(NG => 1), 𝒫ᵉᵐ₀,Dict("InvestmentModels" => extra_inv_data(FixedProfile(200),FixedProfile(1e12),0,FixedProfile(1e12),FixedProfile(0),ContinuousInvestment()))),  
            EMB.RefSource(4, FixedProfile(0), FixedProfile(9), FixedProfile(100), Dict(Coal => 1), 𝒫ᵉᵐ₀,Dict("InvestmentModels" => extra_inv_data(FixedProfile(400),FixedProfile(1e12),0,FixedProfile(1e12),FixedProfile(0),ContinuousInvestment()))),  
          ...
          #  EMB.RefStorage(9, FixedProfile(0), FixedProfile(0), FixedProfile(0), FixedProfile(0),  Dict(CO2 => 1, Power => 0.02), Dict(CO2 => 1),Dict("InvestmentModels" => extra_inv_data_storage(FixedProfile(0),FixedProfile(12),3,FixedProfile(3),FixedProfile(3),FixedProfile(0),FixedProfile(20),5,FixedProfile(5),FixedProfile(5),ContinuousInvestment()))),
          #  EMB.RefGeneration(10, FixedProfile(1000), FixedProfile(0), FixedProfile(0),  Dict(Coal => 2.5), Dict(Power => 1, CO2 => 1), 𝒫ᵉᵐ₀, 0,Dict("InvestmentModels" => extra_inv_data(FixedProfile(1e12),FixedProfile(1000),0,FixedProfile(1000),FixedProfile(0),ContinuousInvestment()))),  
